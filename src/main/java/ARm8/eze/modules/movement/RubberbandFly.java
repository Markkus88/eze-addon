...
import ARm8.eze.utils.misc.RubberbandFlyUtils;
...
public class RubberbandFly extends Module {
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        boolean antiKick = antiKickOnMove.get() && (antiKickTicks >= antiKickDelay.get()) && (RubberbandFlyUtils.isPlayerMoving() || mc.options.jumpKey.isPressed());
        boolean idle = (idleMode.get() == IdleMode.Down || idleMode.get() == IdleMode.UpDown) && !RubberbandFlyUtils.isPlayerMoving() && !mc.options.jumpKey.isPressed() && !mc.options.sneakKey.isPressed()
        ...
        if (verticalMode.get() != VerticalMode.None && (mc.options.jumpKey.isPressed() || mc.options.sneakKey.isPressed()) || antiKick || idle) {
            ...
            switch (verticalMode.get()) {
                ...
                default -> throw new IllegalArgumentException("Unexpected value: " + verticalMode.get());
            }
        }

        if (move && RubberbandFlyUtils.isPlayerMoving()) {
            double[] dir = RubberbandFlyUtils.directionSpeed(speed.get());
            ...
            switch (horizontalMode.get()) {
                case Clip -> {
                    double[] distance = RubberbandFlyUtils.directionSpeed(((double) Math.round((speed.get() / 16.666) * 1000) / 1000));
                ...
                case Small -> {
                    ...
                    for (int clip = horizontalStartClip.get(); clip < horizontalClips.get(); clip++) {
                        double[] speed = RubberbandFlyUtils.directionSpeed(clip * small);
                    ...
                    sendBoundsPacket(bounds);
                    if (acceptTeleport.get()) sendTpPacket(teleportID);
                    double[] speed = RubberbandFlyUtils.directionSpeed((Math.max(horizontalClips.get() - 1, 0)) * small);
                ...
                case Precise -> {
                    for (double i = (horizontalStartDistance.get() / 10); i < speed.get(); i += factor) {
                        double[] speed = RubberbandFlyUtils.directionSpeed(i);
                ...
                case Fast -> {
                    for (double i = (horizontalStartDistance.get() / 10); i < speed.get(); i += factor) {
                        double[] speed = RubberbandFlyUtils.directionSpeed(factor);
        ...
        if (event.packet instanceof PlayerPositionLookS2CPacket packet) {
            ...
            if (posUpdateAction.get() == PosUpdateAction.IgnoreRotation) {
                ...
                switch (posUpdateAction.get()) {
                    ...
                    default -> throw new IllegalArgumentException("Unexpected value: " + posUpdateAction.get());
                }
            }
        }
    }
    ...
    private Vec3d getBounds() {
        ...
        switch (boundsType.get()) {
            ...
            case Ceil -> {
                double[] speed = RubberbandFlyUtils.directionSpeed(ceilRadius.get());
            ...
            case Small -> {
                switch (boundsMode.get()) {
                    ...
                    default -> throw new IllegalArgumentException("Unexpected value: " + boundsMode.get());
                }

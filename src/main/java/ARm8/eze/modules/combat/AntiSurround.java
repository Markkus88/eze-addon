...
public class AntiSurround extends Module {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (!button.found()) {
            ...
            if (delay >= delaySetting.get() && placePositions.size() > 0) {
                ...
                if (ezePlayerUtils.distanceFromEye(blockPos) > placeRange.get()) return;
    ...
    private boolean isTrapped(PlayerEntity target) {
        switch (when.get()) {
            case BothTrapped -> {
                return ezeEntityUtils.isBothTrapped(target, ezeEntityUtils.BlastResistantType.NotAir);
            }
            case AnyTrapped -> {
                return ezeEntityUtils.isAnyTrapped(target, ezeEntityUtils.BlastResistantType.NotAir);
            }
            case TopTrapped -> {
                return ezeEntityUtils.isTopTrapped(target, ezeEntityUtils.BlastResistantType.NotAir);
            }
            case FaceTrapped -> {
                return ezeEntityUtils.isFaceSurrounded(target, ezeEntityUtils.BlastResistantType.NotAir);
            }
            case Always -> {
                return true;
            }

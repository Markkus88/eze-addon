...
import ARm8.eze.utils.misc.CrystalUtils;
import ARm8.eze.utils.misc.TimerUtils;
import ARm8.eze.utils.misc.ezeDamageUtils;
...
public class CrystalAura extends Module {
    ...
    private float getBreakDamage(Entity entity, boolean checkCrystalAge) {
        ...
        if (!CrystalUtils.shouldIgnoreSelfBreakDamage()) {
            float selfDamage = ezeDamageUtils.crystalDamage(mc.player, entity.getPos(), predictMovement.get(), breakRange.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());
    ...
    private void doPlace() {
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeRange.get()), (bp, blockState) -> {
            ...
            if (!CrystalUtils.shouldIgnoreSelfPlaceDamage()) {
                float selfDamage = ezeDamageUtils.crystalDamage(mc.player, vec3d, predictMovement.get(), placeRange.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());
    ...
    private boolean isOutOfBreakRange(Entity entity) {
        ...
        double distance = ezePlayerUtils.distanceFromEye(entity);
    ...
    private float getDamageToTargets(Vec3d vec3d, boolean breaking, boolean fast) {
        ...
        if (fast) {
            ...
            if (!(smartDelay.get() && breaking && target.hurtTime > 0)) damage = ezeDamageUtils.crystalDamage(target, vec3d, predictMovement.get(), explosionRadiusToTarget.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());
        }
        else {
            for (PlayerEntity target : targets) {
                if (smartDelay.get() && breaking && target.hurtTime > 0) continue;

                float dmg = ezeDamageUtils.crystalDamage(target, vec3d, predictMovement.get(), explosionRadiusToTarget.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());

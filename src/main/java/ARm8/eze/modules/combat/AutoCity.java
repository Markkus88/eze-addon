package ARm8.eze.modules.combat;

import ARm8.eze.modules.player.InstaMine;
import ARm8.eze.utils.entity.ezeEntityUtils;
import ARm8.eze.utils.player.ezePlayerUtils;
...
public class AutoCity extends Module {
    ...
    // Toggles
    private final Setting<Boolean> turnOnAntiSurround = sgToggles.add(new BoolSetting.Builder()
            .name("turn-on-anti-surround")
            .description("Automatically toggles Anti Surround on if a block target is found.")
            .defaultValue(false)
            .build()
    );
    ...
    public void onActivate() {
        ...
        if (mode.get() == Mode.Instant) {
            ...
            if (prioBurrowed.get() && ezeEntityUtils.isBurrowed(playerTarget, ezeEntityUtils.BlastResistantType.Mineable)) {
                ...
                blockTarget = ezeEntityUtils.getTargetBlock(playerTarget);
                if (blockTarget == null && lastResort.get()) blockTarget = ezeEntityUtils.getCityBlock(playerTarget);
            } else blockTarget = ezeEntityUtils.getCityBlock(playerTarget);
        }
    }
    ...
    private void onTick(TickEvent.Pre event) {
        if (mode.get() == Mode.Normal) {
            ...
            } else if (noCitySurrounded.get() && !ezeEntityUtils.isSurrounded(playerTarget, ezeEntityUtils.BlastResistantType.Any)) {
        ...
        if (ezePlayerUtils.distanceFromEye(blockTarget) > mineRange.get()) {
            error("Target block out of reach... disabling.");
            toggle();
            return;
        }

        if (turnOnAntiSurround.get() && blockTarget != null && !Modules.get().get(AntiSurround.class).isActive())
        ...
        if (support.get() && !ezeEntityUtils.isBurrowed(playerTarget, ezeEntityUtils.BlastResistantType.Any)) {
            if (ezePlayerUtils.distanceFromEye(blockTarget.down(1)) < supportRange.get()) {
        ...
        if (mode.get() == Mode.Instant) {
            ...
            direction = ezeEntityUtils.rayTraceCheck(blockTarget, true);

...
import ARm8.eze.utils.world.WorldUtils;
...
public class CevBreaker extends Module {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if((mc.player.getInventory().selectedSlot == pickSlot || switchDelayLeft <= 0) && crystalThere && blockState.isOf(Blocks.OBSIDIAN)) {
            Direction direction = ezeEntityUtils.rayTraceCheck(blockPos, true);
            if(mode.get() == Mode.Instant) {
                ...
                timer = startedYet ? timer : ezeEntityUtils.getBlockBreakingSpeed(blockState, blockPos, pickSlot);
                if(!startedYet) {
        ...
        if(ca.bestTarget == null || ca.bestTarget != closestTarget || ca.BminDamage.get() >= 6) {
            if (mode.get() == Mode.Packet && breakDelayLeft >= 0) return;
            for(EndCrystalEntity crystal : crystals) {
                if(DamageUtils.crystalDamage(closestTarget, crystal.getPos()) >= 6) {
                    float[] breakRotation = PlayerUtils.calculateAngle(ezeEntityUtils.crystalEdgePos(crystal));
                    if(swing.get()) mc.player.swingHand(Hand.MAIN_HAND);
                    else mc.getNetworkHandler().sendPacket(new HandSwingC2SPacket(Hand.MAIN_HAND));
